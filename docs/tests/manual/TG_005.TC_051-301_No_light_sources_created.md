...
### Purpose
 The purpose of this test is to check that solution committed to issue no. #301 will not create any Minecraft object that could be used as a light source (torches, glowstones, etc. for complete list: https://minecraft.fandom.com/wiki/Light#Light-emitting_blocks) around the monster spawner block (https://minecraft.fandom.com/wiki/Monster_Spawner) that would prevent monsters from spawning. **These light source blocks only include blocks created by this issue's solution. Redstone lamps around doors were created by an older issue.**
