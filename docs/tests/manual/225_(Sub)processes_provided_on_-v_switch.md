...
### Purpose
The purpose of this test is to check if the provdied messages that appear in the console (when running the solution to issue no. #225) contain **(sub)processes**. The tool that needs to be checked for this is the converter and it has to be ran in *verbose* mode.
...
### Expected Result
The messages that appear in the console contain the (sub)processes used during the tool's runtime.