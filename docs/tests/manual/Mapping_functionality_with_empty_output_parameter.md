# Test mapping with empty output parameter

## Tags
#component_mapping #testFile-format_XML  #category_output #category_error_handling

### Purpose
This test assesses the mapping tool's behavior when the output file parameter is empty, ensuring it alerts the user about the missing parameter and exits without generating an output file.
...
1. Run:
	```cmd
	java -jar mapping-1.5.0.jar -i converterToMapping.xml -m sourcemeter_mapping_example_2_1.xml -o
	``` 
