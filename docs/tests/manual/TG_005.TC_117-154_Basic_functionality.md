...
### Pre-requisites
- The command-line tool for issue #154 is available.
- An IXML file with known properties that can be targeted with regex patterns is prepared.

### Steps
1. Open a command-line interface.
2. Navigate to the directory where the tool and the IXML file are located.
...
```cmd 
   java -jar ixmlexclude.jar -i input.ixml -o output.ixml --exclude-property-name "examplePropertyName" --exclude-property-value "examplePropertyValue"
```
5. Wait for the process to complete and verify the output.
