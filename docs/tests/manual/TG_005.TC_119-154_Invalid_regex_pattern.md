...
2. Navigate to the directory containing the tool and the IXML file.
3. Execute the tool with an invalid regex pattern for either the property name or value. Example command: 
```cmd
java -jar ixmlexclude.jar -i input.ixml -o output.ixml --exclude-property-name "*" --exclude-property-value "*(invalid regex)"
```
