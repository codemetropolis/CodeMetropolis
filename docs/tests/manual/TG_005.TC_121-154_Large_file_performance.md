...
- A large IXML file (e.g., several GBs in size) is available for testing.

### Steps
1. Open a command-line interface.
2. Navigate to the directory containing the tool and the large IXML file.
3. Execute the tool with the large IXML file as input. Example command: 
```cmd
java -jar ixmlexclude.jar -i large.ixml -o output.ixml
```
