...
//to run: -t gitlab -i hostUrl (e.g.: http://gitlab-okt.sed.hu) -o output_dir -p username=<your_username> password=<your_password> group=<group_name> project=<project_name> projectID=<project_id>
...
public class GitLabConverter extends CdfConverter {
    ...
    public CdfTree createElements(String source) throws CodeMetropolisException {
        ...
        try {
            ...
            {
                if(pr.getId().equals(Integer.parseInt(getParameter(PROJECT_ID)))) {
                    p = pr;
