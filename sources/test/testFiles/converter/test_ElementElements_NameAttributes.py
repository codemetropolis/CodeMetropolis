...
def testElementElements_NameAttributes(expected, output):
    ...
    try:
        ...
        for i in range(expectedListLength):
            if(expectedNameAttributesList[i] != outputNameAttributesList[i]):
                ...
                break
        assert expectedListLength == passCounter, f"test_ElementElements_NameAttributes: The value of the element element name attribute is '{errorName}' in the output file, but it should be '{correctName}' based on the expected file."
