...
/**
 * A subclass for {@link House} with a little modification for {@link Buildable.Type#CELLAR} floors.
 * So cellars are not under the ground.
 * 
...
public class TownHouse extends House {
	...
	/**
	 * Parents size is no more subtracted.
	 * 
	 * {@inheritDoc}
	 */
	@Override
	...
	/**
	 * Cellars are added similarly to floors, so they won't be under the ground.
	 * Height is maximum one floor height.
	 * 
	 * {@inheritDoc}
	 */
	@Override
	public boolean addCellar(Buildable cellar) throws LayoutException {
		...
		if(cellar.getParent() != getParent()) {
			throw new LayoutException(String.format("Cannot add %s %d as a cellar. Floors and cellars in the house must be childs of the same parent.", cellar.getType().toString(), cellar.getId()));
		}
		if(getTopFloor().getPositionY() + getTopFloor().getSizeY() + cellar.getSizeY() > maxHeight) return false;
		if(cellar.getSizeX() > getTopFloor().getSizeX() || cellar.getSizeZ() > getTopFloor().getSizeZ()) return false;
