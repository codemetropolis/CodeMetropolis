...
/**
 * A {@link Garden} subclass for the {@link Themes#TOWN} theme.
 * A nice garden with flowers and trees, surrounded by a hedge.
 * 
...
public class TownGarden extends Garden {
	...
	/**
	 * A nice garden with flowers and trees.
	 */
	@Override
	...
	protected void prepareDoor() {
		...
		primitives.add(	new SimpleBox( position.translate( new Point( center.getX(), 1, 0 ) ), new Point( 1, 2, 1 ), space, Orientation.NearX ) );
		primitives.add( new SimpleBox( position.translate( new Point( center.getX(), 1, size.getZ() - 1) ), new Point( 1, 2, 1 ), space, Orientation.NearX ) );
		primitives.add( new SimpleBox( position.translate( new Point( 0, 1, center.getZ() ) ), new Point( 1, 2, 1 ), space, Orientation.NearX ) );
		primitives.add( new SimpleBox( position.translate( new Point( size.getX() - 1, 1, center.getZ() ) ), new Point( 1, 2, 1 ), space, Orientation.NearX ) );
	}
	 ...
	 */
	@Override
	protected void prepareSigns() {primitives.add(new WallSign(position.getX(), position.getY() + 2, position.getZ() - 1, WallSign.Orientation.NORTH, innerBuildable.getName()));
		primitives.add(new WallSign(position.getX() + size.getX() - 1, position.getY() + 2, position.getZ() - 1, WallSign.Orientation.NORTH, innerBuildable.getName()));
		primitives.add(new WallSign(position.getX(), position.getY() + 2, position.getZ() + size.getZ(), WallSign.Orientation.SOUTH, innerBuildable.getName()));
		primitives.add(new WallSign(position.getX() + size.getX() - 1, position.getY() + 2, position.getZ() + size.getZ(), WallSign.Orientation.SOUTH, innerBuildable.getName()));
	}
